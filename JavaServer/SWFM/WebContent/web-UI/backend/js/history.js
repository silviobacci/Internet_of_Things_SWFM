class History {
	...
	add_data_point(data_points) {
		for(var i = 0; i < data_points.length; i++)
			this.chart.options.data[0].dataPoints.push(this.data_points[i]);
	}
	...
	build_chart() {
	    ...
	    this.chart = new CanvasJS.Chart(this.container_name, {
	        ...
	        axisX: {
	            ...
	            gridThickness: 0,
	            interval: 1, 
	            labelFormatter: function (e) {return CanvasJS.formatDate(self.dates[e.value], "DD MMM");}
	        },
	        axisY: {
	            gridThickness: 0
	        },
	        toolTip: {
	        		contentFormatter	: function (e) {return CanvasJS.formatDate(self.dates[e.entries[0].dataPoint.x], "DD MMM") + " : " + e.entries[0].dataPoint.y;}
	        },
	        data: [{
		...
		if(reply.error == false) {
			...
			for(var i = 0; i < reply.message.length; i++) {
				if(reply.message[i] != undefined && reply.message[i] != null) {
					this.data_points.push({x: start_counter + i, y: reply.message[i].y})
					new_data_points.push({x: start_counter + i, y: reply.message[i].y});
